...
function printHelpAndExit() {
  console.log(`
Usage:
  npm run ykt -- --url "https://oucbk.yuketang.cn/pro/lms/<course>/<classroom>/exam/<leaf>" --loop --stable 3 --auto-fill --auto-submit --unknown-policy random
...
function parseQuizRoute(urlValue) {
  ...
  try {
    ...
    meta.origin = "https://oucbk.yuketang.cn";
  }
  return meta;
}
...
function isQuestionScreenText(text) {
  ...
  if (/单选题|多选题|判断题|填空题|下一题|上一题|交卷|提交答案/.test(normalized)) return true;
  if (/^[\s\S]*\b[A-H]\s*[.．、:：\)]\s*\S+[\s\S]*\b[B-H]\s*[.．、:：\)]\s*\S+/.test(normalized)) {
    return !/试卷分数|题数|考生须知|开始答题/.test(normalized);
  }
  return false;
}
